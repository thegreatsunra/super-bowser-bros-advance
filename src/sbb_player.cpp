...
Player::Player(bn::sprite_ptr sprite)
		: m_camera(bn::camera_ptr::create(0, 0)), m_map(bn::affine_bg_items::stage.create_bg(0, 0)), m_sprite(sprite) {
...
void Player::t_apply_animation_state() {
	if (m_is_jumping) {
		...
		if (m_action.graphics_indexes().front() != 0) {
			m_action = bn::create_sprite_animate_action_forever(m_sprite, 30, bn::sprite_items::bowser_sprite.tiles_item(), 0,
																													0, 0, 0);
		}
	}
...
void Player::t_collide_with_objects(sbb::Level level, bn::affine_bg_ptr map) {
	// if falling
	if (m_dy > 0) {
...
void Player::t_hide() { m_sprite.set_visible(false); }
...
bn::fixed_point Player::t_pos() { return m_pos; }
...
void Player::t_spawn(bn::camera_ptr camera, bn::vector<Enemy, 16> &enemies, bn::affine_bg_ptr map,
										 bn::fixed_point pos) {
	m_camera = camera;
	m_enemies = &enemies;
	m_map = map;
