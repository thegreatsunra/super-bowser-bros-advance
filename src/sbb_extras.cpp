...
bool hitbox_collided_with_cell(bn::fixed_point pos, directions direction, Hitbox hitbox, bn::affine_bg_ptr &map,
															 sbb::Level level, bn::span<const bn::affine_bg_map_cell> cells) {
	...
	if (contains_cell(get_map_cell(l, u, map, cells), tiles) || contains_cell(get_map_cell(l, d, map, cells), tiles) ||
			contains_cell(get_map_cell(r, u, map, cells), tiles) || contains_cell(get_map_cell(r, d, map, cells), tiles)) {
		return true;
	} else {
		return false;
	}
